...
def test_advisory_influence(sample_transaction, medium_risk_score):
    ...
    # Create high confidence advisory with decay fields
    advisory = Advisory(
        advisory_id="adv_001",
        ...
        fraud_score=85.0,
        timestamp=datetime.now().isoformat(),
        # Add decay fields for fresh, high-confidence pattern
        base_confidence=0.9,
        decay_score=1.0,
        effective_confidence=0.9,  # Fresh pattern
        last_seen_timestamp=datetime.now(),
        pattern_status="ACTIVE",
        time_since_last_seen_seconds=0.0,
        decay_explanation="Fresh pattern detected across multiple entities"
    )
